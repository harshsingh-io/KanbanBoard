...
## 2024-05-23 - [UX: Keyboard Navigation]
**Learning:** Forms often neglect `imeOptions` ("Next", "Done"), forcing users to manually dismiss the keyboard to find the submit button. This is a significant friction point on mobile.
**Action:** Always add `android:imeOptions="actionNext"` to intermediate fields and `android:imeOptions="actionDone"` to the final field. Crucially, pair "actionDone" with an `OnEditorActionListener` in the code to trigger the submission logic.
