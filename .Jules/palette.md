...
## 2024-05-23 - [Android Form UX Pattern]
**Learning:** Adding `imeOptions="actionDone"` to the final form field isn't enough; you must explicitly handle the `EditorInfo.IME_ACTION_DONE` in the Activity.
**Action:** Use the pattern `binding?.etField?.setOnEditorActionListener { _, actionId, _ -> if (actionId == EditorInfo.IME_ACTION_DONE) { binding?.btnSubmit?.performClick(); true } else false }` to reuse existing button logic and delight users.
